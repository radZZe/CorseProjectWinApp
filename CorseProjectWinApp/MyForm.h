...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->btnAddClient->Text = L"ƒобавить";
			...
			this->btnSearchClientByEmail->Text = L"ѕо почте";
			...
			this->btnSearchByClientJob->Text = L"ѕо должности";
			...
			this->button8->Text = L"ѕо пасспорту";
			...
			this->btnSearchByClient->Text = L"ѕо полным данным";
			...
			this->debugClientButton->Text = L"ќкно отладки";
			...
			this->label9->Text = L" оличество сравнений: ";
			// 
			// label10
			// 
			...
			this->btnDeletionClient->Text = L"”далить";
	...
	private: System::Void FormLoad(System::Object^ sender, System::EventArgs^ e) {
		...
		tablClients->Columns->Add("‘»ќ");
		tablClients->Columns->Add("ƒолжность");
		tablClients->Columns->Add("email");
		tablClients->Columns->Add("ѕасспорт");
		for (int i = 0; i < dataClients.size(); i++) {
			DataRow^ rowClients = tablClients->NewRow();
			rowClients["‘»ќ"] = gcnew String((dataClients[i]->fullname.surname +' '+ dataClients[i]->fullname.name + ' ' + dataClients[i]->fullname.lastname).c_str());
			rowClients["ƒолжность"] = gcnew String(dataClients[i]->job.c_str());
			...
			rowClients["ѕасспорт"] = gcnew String(passportClient.c_str());
	...
	if (serviceName.size() == 0 || serviceType.size() == 0 || dateString.size() == 0 || seriesPassportText.size() == 0 || numberPassportText.size() == 0) {
		...
		try {
			...
			if (result_searchByTypeAndNameOfService[1] != -1) {
				this->resultSearch->Text = gcnew String("Ќайдено");
			}
			else {
				this->resultSearch->Text = gcnew String("Ќе найдено");
			}
		}
		catch (exception& err) {
			MessageBox::Show(this, "Iaei??aeoiua aaiiua , i?iaa?uoa aaaaaiio? eioi?iaoe? i cayaea", "Oaaeaiea", MessageBoxButtons::OK, MessageBoxIcon::Error);
...
private:System::Void searchByClientPassport_Click(System::Object^ sender, System::EventArgs^ e)
    {
        ...
        if (result) {
            this->resultSearchClient->Text = gcnew String("Ќайдено");
        }
        else {
	...
	if (!isValidateData(marshal_as<std::string>(this->tBClientPassNum->Text), marshal_as<std::string>(this->tBClientPassSeries->Text),fullnamestr,email,job)) {
		...
		try {
			...
			if (result) {
				...
				this->resultSearchClient->Text = gcnew String("Ќе найдено");
			}
			/*print(rbt, DataClientsStorage::rbtNullnode, 4, 2);
			cout << endl;*/
		}
...
private: System::Void btnAddClient_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	{
		MessageBox::Show(this, "Ќекорректные данные, проверьте правильности введЄнных данных", "ќшибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
	}
	passport.number = passnum;
	passport.series = passseries;
	string email = marshal_as<std::string>(this->tBClientEmai->Text);
	...
	if (!isValidateData(marshal_as<std::string>(this->tBClientPassNum->Text), marshal_as<std::string>(this->tBClientPassSeries->Text), fullnamestr, email, job)) {
		...
		else if (!isContain)
		{
			try
			{
				...
				MessageBox::Show(this, " лиент успешно добавлен", "ƒобавление", MessageBoxButtons::OK, MessageBoxIcon::Information);
				DataTable^ tablTestClient = gcnew DataTable();
				tablTestClient->Rows->Clear();
				ClientsDataGrid->DataSource = tablTestClient;
				tablTestClient->Columns->Add("‘»ќ");
				tablTestClient->Columns->Add("ƒолжность");
				tablTestClient->Columns->Add("email");
				tablTestClient->Columns->Add("ѕасспорт");
				for (int i = 0; i < DataClientsStorage::data.size(); i++) {
					DataRow^ rowClients = tablTestClient->NewRow();
					rowClients["‘»ќ"] = gcnew String((DataClientsStorage::data[i]->fullname.surname + ' ' + DataClientsStorage::data[i]->fullname.name + ' ' + DataClientsStorage::data[i]->fullname.lastname).c_str());
					rowClients["ƒолжность"] = gcnew String(DataClientsStorage::data[i]->job.c_str());
			...
			MessageBox::Show(this, "“акой клиент уже есть в справочнике", "ƒобавление", MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
	}
}
		...
		try 
		{
			if (isContains) {
				...
				if (result_searchByPassportTreeNode) {
					...
					MessageBox::Show(this, " лиент успешно удален", "”даление", MessageBoxButtons::OK, MessageBoxIcon::Information);
					DataTable^ tablTestClient = gcnew DataTable();
					tablTestClient->Rows->Clear();
					ClientsDataGrid->DataSource = tablTestClient;
					tablTestClient->Columns->Add("‘»ќ");
				...
				MessageBox::Show(this, "“акого клиента нет в справочнике", "ќшибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
			}
		}
		catch (exception& err) {
			MessageBox::Show(this, "Ќекорректные данные, проверьте правильность введЄнных данных", "ќшибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
...
private: System::Void debugClientButton_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	fout.close(); 
	MessageBox::Show(this, "—труктуры данных сохранены в файл debugClients.txt", "ќтладка", MessageBoxButtons::OK, MessageBoxIcon::Information);

}
private: System::Void button20_Click(System::Object^ sender, System::EventArgs^ e) {
	vector<ClientsEntity*> data = DataClientsStorage::data;
	writeDataClient(data, "stdclients1.txt");
	MessageBox::Show(this, "ƒанные клиентов успешно сохранены", "—охрание", MessageBoxButtons::OK, MessageBoxIcon::Information);
