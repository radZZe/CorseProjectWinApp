...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->btnAddClient->Text = L"Äîáàâèòü";
			...
			this->btnSearchClientByEmail->Text = L"Ïî ïî÷òå";
			...
			this->btnSearchByClientJob->Text = L"Ïî äîëæíîñòè";
			...
			this->btnSearchByClient->Text = L"Ïî ïîëíûì äàííûì";
			...
			this->debugClientButton->Text = L"Îêíî îòëàäêè";
			...
			this->btnDeletionClient->Text = L"Óäàëèòü";
	...
	private: System::Void FormLoad(System::Object^ sender, System::EventArgs^ e) {
		...
		tablClients->Columns->Add("?Y?�N?N?????N�N�");
		for (int i = 0; i < dataClients.size(); i++) {
			...
			rowClients["?Y?�N?N?????N�N�"] = gcnew String(passportClient.c_str());
...
private:System::Void searchByClientPassport_Click(System::Object^ sender, System::EventArgs^ e)
    {
        ...
        if (result) {
            this->resultSearchClient->Text = gcnew String("Íàéäåíî");
        }
        else {
	...
	if (!isValidateData(marshal_as<std::string>(this->tBClientPassNum->Text), marshal_as<std::string>(this->tBClientPassSeries->Text),fullnamestr,email,job)) {
		...
		try {
			...
			if (result) {
				...
				this->resultSearchClient->Text = gcnew String("Íå íàéäåíî");
			}
			/*print(rbt, DataClientsStorage::rbtNullnode, 4, 2);
			cout << endl;*/
		}
...
private: System::Void btnAddClient_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	{
		MessageBox::Show(this, "Íåêîððåêòíûå äàííûå, ïðîâåðüòå ïðàâèëüíîñòè ââåä¸ííûõ äàííûõ", "Îøèáêà", MessageBoxButtons::OK, MessageBoxIcon::Error);
	}
	passport.number = passnum;
	passport.series = passseries;
	string email = marshal_as<std::string>(this->tBClientEmai->Text);
	...
	if (!isValidateData(marshal_as<std::string>(this->tBClientPassNum->Text), marshal_as<std::string>(this->tBClientPassSeries->Text), fullnamestr, email, job)) {
		...
		else if (!isContain)
		{
			try
			{
				...
				MessageBox::Show(this, "Êëèåíò óñïåøíî äîáàâëåí", "Äîáàâëåíèå", MessageBoxButtons::OK, MessageBoxIcon::Information);
				DataTable^ tablTestClient = gcnew DataTable();
				tablTestClient->Rows->Clear();
				ClientsDataGrid->DataSource = tablTestClient;
				tablTestClient->Columns->Add("ÔÈÎ");
				tablTestClient->Columns->Add("Äîëæíîñòü");
				tablTestClient->Columns->Add("email");
				tablTestClient->Columns->Add("Ïàññïîðò");
				for (int i = 0; i < DataClientsStorage::data.size(); i++) {
					DataRow^ rowClients = tablTestClient->NewRow();
					rowClients["ÔÈÎ"] = gcnew String((DataClientsStorage::data[i]->fullname.surname + ' ' + DataClientsStorage::data[i]->fullname.name + ' ' + DataClientsStorage::data[i]->fullname.lastname).c_str());
					rowClients["Äîëæíîñòü"] = gcnew String(DataClientsStorage::data[i]->job.c_str());
					...
					//rowClients["Ianii?oiua aaiiua"] = gcnew String(passport.c_str());
					rowClients["Ïàññïîðò"] = gcnew String(passportClient.c_str());
			...
			MessageBox::Show(this, "Òàêîé êëèåíò óæå åñòü â ñïðàâî÷íèêå", "Äîáàâëåíèå", MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
	}
...
private: System::Void saveDebugServices_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	fout.close();
	MessageBox::Show(this, "��������� ����� ������� ��������� � ���� debugServices.txt", "�������", MessageBoxButtons::OK, MessageBoxIcon::Information);
}
...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	{
		MessageBox::Show(this, "???�????N�N�?�??N�??N�?� ???�????N�?�, ??N�?????�N�N?N�?� ?????�???�????N?NZ ????N�??N�???�N�??NZ ?? ???�???�??N�?�", "?zN????�???�", MessageBoxButtons::OK, MessageBoxIcon::Error);
	}
	passport.number = passnum;
	passport.series = passseries;
	string email = marshal_as<std::string>(this->tBClientEmai->Text);
		...
		try 
		{
			if (isContains) {
				...
				if (result_searchByPassportTreeNode) {
					...
					MessageBox::Show(this, "Êëèåíò óñïåøíî óäàëåí", "Óäàëåíèå", MessageBoxButtons::OK, MessageBoxIcon::Information);
					DataTable^ tablTestClient = gcnew DataTable();
					tablTestClient->Rows->Clear();
					ClientsDataGrid->DataSource = tablTestClient;
					tablTestClient->Columns->Add("ÔÈÎ");
				...
				MessageBox::Show(this, "Òàêîãî êëèåíòà íåò â ñïðàâî÷íèêå", "Îøèáêà", MessageBoxButtons::OK, MessageBoxIcon::Error);
			}
		}
		catch (exception& err) {
			MessageBox::Show(this, "Íåêîððåêòíûå äàííûå, ïðîâåðüòå ïðàâèëüíîñòü ââåä¸ííûõ äàííûõ", "Îøèáêà", MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
...
private: System::Void debugClientButton_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	fout.close(); 
	MessageBox::Show(this, "?�?�????N�?� N???N�N�?�???�??N� ?? N�?�???� debugClients.txt", "??N????�N?????", MessageBoxButtons::OK, MessageBoxIcon::Information);

}
private: System::Void button20_Click(System::Object^ sender, System::EventArgs^ e) {
	vector<ClientsEntity*> data = DataClientsStorage::data;
	writeDataClient(data, "stdclients1.txt");
	MessageBox::Show(this, "Äàííûå êëèåíòîâ óñïåøíî ñîõðàíåíû", "Ñîõðàíèå", MessageBoxButtons::OK, MessageBoxIcon::Information);
